    ...
    if (type === "test") {
      ...
      ({ subject, html } = renderWeeklySummaryEmail({
        ...
        topChanges: allChanges.map((c) => ({
          ...
          newAction: c.newAction, priorTargetShares: c.priorTargetShares,
          newTargetShares: c.newTargetShares, sharesDelta: c.sharesDelta,
          priorWeight: c.priorWeight, newWeight: c.newWeight,
          changed: c.changed, changeReason: c.changeReason,
        })),
        recommendations: latestReport?.recommendations ?? [],
        profile: user.profile!,
        appUrl,

...
import { fetchPriceTimelines, fetchPriceTimelinesDetailed } from "@/lib/research/price-timeline";
...
describe("runtime cache price timeline", () => {
  ...
  test("reports cache-hit diagnostics for intraday bounded refresh", async () => {
    const fetchMock = jest.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        chart: {
    ...
    const articleMap = new Map<string, { title: string; publishedAt: string }[]>();

    await fetchPriceTimelinesDetailed(["AAPL"], articleMap, "2026-04-02", () => {});
    const second = await fetchPriceTimelinesDetailed(["AAPL"], articleMap, "2026-04-02", () => {});

    expect(second.diagnostics.providerCallCount).toBe(0);
    expect(second.diagnostics.cacheHitCount).toBe(1);
    expect(second.diagnostics.cacheMissCount).toBe(0);
